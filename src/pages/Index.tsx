        ...
        <div className="relative container mx-auto px-4 pt-24">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="max-w-2xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-sm font-medium text-primary uppercase tracking-widest">September 2026 · Kerala</span>
            </div>
            <h1 className="text-5xl md:text-7xl font-bold leading-[1.05] mb-6">
              <span className="text-foreground">Gateway to</span><br /><span className="text-gradient">Greatness</span>
            </h1>
            ...
            <div className="flex flex-wrap gap-3">
              <Link to="/events"><Button size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-glow gap-2">Explore Events <ArrowRight size={18} /></Button></Link>
              <Link to="/about"><Button size="lg" variant="outline" className="border-border text-foreground hover:bg-secondary">Learn More</Button></Link>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-3">Sport Categories</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {sportCategories.map((cat, i) => (
              <motion.div key={cat.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}>
                <Link to={`/events?category=${cat.id}`} className="group flex flex-col items-center gap-3 p-6 rounded-xl border border-border bg-card hover:border-primary/50 hover:shadow-glow transition-all">
        ...
        <section className="py-20 bg-khelium-navy">
          <div className="container mx-auto px-4">
            <div className="flex items-end justify-between mb-10">
              ...
              <Link to="/events"><Button variant="ghost" className="text-primary hover:text-primary/80 gap-1">View All <ArrowRight size={16} /></Button></Link>
            </div>
      ...
      <section className="py-24">
        <div className="container mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-5xl font-bold text-foreground mb-4">One Event. One Dream. <span className="text-gradient">One Vision.</span></h2>
            <p className="text-lg text-muted-foreground max-w-xl mx-auto mb-8">Every Sport, Every Spark 🔥 Register your child and ignite their path to Olympic excellence.</p>
            <Link to="/events"><Button size="lg" className="bg-primary text-primary-foreground hover:bg-primary/90 shadow-glow gap-2">Register Now <Trophy size={18} /></Button></Link>
          </motion.div>
        </div>
      </section>
    </div>
