          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
              <div className="flex items-center gap-3 p-4 rounded-lg bg-card border border-border">
                ...
                <div><p className="text-sm text-muted-foreground">Date & Time</p><p className="text-sm font-medium text-foreground">{event.event_date} · {event.event_time}</p></div>
              </div>
              <div className="flex items-center gap-3 p-4 rounded-lg bg-card border border-border">
                <MapPin size={20} className="text-primary" />
                <div><p className="text-sm text-muted-foreground">Venue</p><p className="text-sm font-medium text-foreground">{event.venue}, {event.city}</p></div>
              </div>
              <div className="flex items-center gap-3 p-4 rounded-lg bg-card border border-border">
                <Users size={20} className="text-primary" />
                <div><p className="text-sm text-muted-foreground">Age Groups</p><p className="text-sm font-medium text-foreground">{event.age_groups.join(", ")}</p></div>
              </div>
              <div className="flex items-center gap-3 p-4 rounded-lg bg-card border border-border">
                <Clock size={20} className="text-primary" />
                <div><p className="text-sm text-muted-foreground">Availability</p><p className="text-sm font-medium text-foreground">{spotsLeft} of {event.total_spots} spots left</p></div>
              </div>
            </div>
            ...
            <div className="sticky top-24 rounded-xl border border-border bg-card p-6 shadow-card">
              ...
              {!user ? (
                <div className="text-center py-6">
                  ...
                  <Link to="/auth"><Button className="bg-primary text-primary-foreground w-full">Sign In / Sign Up</Button></Link>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div><Label className="text-foreground text-xs">Child's Name</Label><Input required value={formData.childName} onChange={(e) => setFormData({ ...formData, childName: e.target.value })} className="bg-muted border-border text-foreground" placeholder="Full name" /></div>
                  <div><Label className="text-foreground text-xs">Parent's Name</Label><Input required value={formData.parentName} onChange={(e) => setFormData({ ...formData, parentName: e.target.value })} className="bg-muted border-border text-foreground" placeholder="Full name" /></div>
                  <div><Label className="text-foreground text-xs">Email</Label><Input required type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="bg-muted border-border text-foreground" placeholder="parent@email.com" /></div>
                  <div><Label className="text-foreground text-xs">Phone</Label><Input required value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} className="bg-muted border-border text-foreground" placeholder="+91 ..." /></div>
                  <div><Label className="text-foreground text-xs">School</Label><Input required value={formData.school} onChange={(e) => setFormData({ ...formData, school: e.target.value })} className="bg-muted border-border text-foreground" placeholder="School name" /></div>
                  ...
                  <Button type="submit" disabled={submitting} className="w-full bg-primary text-primary-foreground hover:bg-primary/90 shadow-glow" size="lg">
