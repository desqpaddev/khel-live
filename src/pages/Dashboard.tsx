...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const [profileRes, regRes, resultsRes] = await Promise.all([
    ...
    if (profileRes.data) {
      ...
      setEditProfile({
        ...
        age_group: profileRes.data.age_group || "",
      });
    }
            ...
            {registrations.length === 0 ? (
              ...
              <div className="grid gap-4">
                {registrations.map((reg) => (
                  ...
                  >
                    <div>
                      <h3 className="font-bold text-foreground">{reg.events?.title || "Event"}</h3>
            ...
            <div className="max-w-lg rounded-xl border border-border bg-card p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input value={editProfile.full_name} onChange={(e) => setEditProfile({ ...editProfile, full_name: e.target.value })} className="bg-muted border-border text-foreground" />
                </div>
                <div>
                  <Label className="text-foreground text-xs">Phone</Label>
                  <Input value={editProfile.phone} onChange={(e) => setEditProfile({ ...editProfile, phone: e.target.value })} className="bg-muted border-border text-foreground" />
                </div>
                <div>
                  <Label className="text-foreground text-xs">School</Label>
                  <Input value={editProfile.school} onChange={(e) => setEditProfile({ ...editProfile, school: e.target.value })} className="bg-muted border-border text-foreground" />
                </div>
                <div>
                  <Label className="text-foreground text-xs">City</Label>
                  <Input value={editProfile.city} onChange={(e) => setEditProfile({ ...editProfile, city: e.target.value })} className="bg-muted border-border text-foreground" />
                </div>
